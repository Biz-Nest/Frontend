...
export default function DashStore({ tokens, onAddStore }) {
  ...
  useEffect(() => {
    if (data) {
      const filteredStores = data.filter(
        (store) => store.owner === tokens.user.id
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/store/${selectedStore.id}/`,
        {
          method: "DELETE",
          headers: {
      ...
      setUserStores((prevStores) =>
        prevStores.filter((store) => store.id !== selectedStore.id)
      );
      setIsPopupOpen(false);
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      Object.keys(selectedStore.social_links).forEach((key) => {
        formData.append(
          `social_links[${key}]`,
          selectedStore.social_links[key] || ""
        );
      });
    ...
    <div className="container">
          ...
          {userStores.map((store) => (
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <h2 className="dark:text-white mb-4 text-2xl font-bold text-gray-800">
                  {store.name}
                </h2>
                <p className="dark:text-[#ddd] text-gray-600">
                  {store.description || "No description available"}
                </p>
                <p className="dark:text-[#ddd] mb-4 text-gray-600"><i className="ri-eye-line"></i> {store.views}</p>
      ...
      {isPopupOpen && selectedStore && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="scale-[0.85] relative w-full max-w-lg p-8 bg-white rounded-lg shadow-lg">
            <button
            ...
            <div className="modal-store mb-4">
              {/* Uncomment to display store logo */}
              {/* {selectedStore.logo && (
                <Image
                  width={200}
