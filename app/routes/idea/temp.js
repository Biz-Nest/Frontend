...
export default function IdeaDetail() {
    ...
    const fetchIdea = async () => {
      ...
      try {
        const data = await fetcher(
          `${process.env.NEXT_PUBLIC_API_URL}/idea/${id}/`,
          storedTokens.access
        );
        setIdea(data);

        const relatedData = await fetcher(
          `${process.env.NEXT_PUBLIC_API_URL}/idea/`,
          storedTokens.access
        );
        const filteredRelated = relatedData.filter(
          (currentIdea) =>
            currentIdea.category === data.category && currentIdea.id !== data.id
        );
  ...
  return (
    <>
      <div className="landing bg-white shadow-lg rounded-lg p-12">
          ...
          <div className="grid grid-cols-1 gap-4 mt-4 md:grid-cols-3">
            {relatedCat.map((relatedIdea, index) => (
              <div key={index} className="p-4 border rounded-lg shadow">
                <p>
                  <strong>Name:</strong> {relatedIdea.name}
                </p>
                <p>
                  <strong>id field for test:</strong> {relatedIdea.id}
                </p>
                <p>
                  <strong>Description:</strong> {relatedIdea.description}
                </p>
                <p>
                  <strong>Category:</strong> {relatedIdea.category}
                </p>
                <p>
                  <strong>Owner:</strong> {relatedIdea.owner}
                </p>
                <p>
                  <strong>Cost:</strong> ${relatedIdea.cost}
                </p>
                <p>
                  <strong>Location:</strong> {relatedIdea.location}
                </p>
                <p>
                  <strong>Expenses:</strong> ${relatedIdea.expenses}
                </p>
                <p>
                  <strong>Date:</strong> {relatedIdea.date}
                </p>
              </div>
            ))}
          </div>
